...
https://github.com/matt-graham/hamiltonian-monte-carlo/blob/master/hmc/portfolio_base.py
    ...
    def get_samples(self, pos, dt, n_step_per_sample, n_sample, mom=None):
        ...
        for s in range(1, n_sample):
            ...
            try:
                ...
                logging.info('Error occured when simulating dynamic. '
                             'Rejecting.\n' + str(e))
...
def pdf_exponential(x, cache={}):
    logp = -0.5 * 1 * x ** 2
    return logp

def grad_pdf_exponential(x, cache={} ):
    # return -exp_lambda**2*np.exp(-1*exp_lambda*x)
    return -1 * x
...
def plot_samples():
    ...
    sampler = EuclideanMetricHmcSampler(pdf_exponential,
                                        np.array([1]),
                                        grad_pdf_exponential)
    samples = sampler.get_samples(np.array([1]), 0.1, 10, 2000)
    print(samples)
    # x = np.linspace(-10, 10, n_sample)
    # y = 0.5 * x * x
    # # print(samples)
    # axes[0].plot(x, y, color="green", lw=2)
    axes[0].hist(samples[0], bins=100, normed=True)
    #
    plt.show()
